...
_SECRETS = {}                # populated once per session from first inject_into_sandbox call
...
def _env_prefix() -> str:
    ...
    for k, v in _SECRETS.items():
        if v:
            parts.append(f"export {k}={json.dumps(v)}")
    return "; ".join(parts) + "; " if parts else ""


def run_worker(dt, sid, wt, model, spec, tag, worker_timeout=480, allowed=None):
    ...
    if allowed:
        gi_lines = ["# auto-generated by orchestrator", "/*"]
        for p in allowed:
            gi_lines.append(f"!{p}")
        gi_content = "\n".join(gi_lines) + "\n"
        encoded = b64(gi_content)
        dt.exec(sid, f"cd \"{wt}\" && echo {encoded} | base64 -d > .gitignore", timeout=15)
    env = _env_prefix()
    if model == OPUS:
        cmd = (f"{env}{OPATH}; cd \"{wt}\" && cat {specfile} | claude -p --model {WORKER_CLAUDE_MODEL} "
        ...
        cmd = (f"{env}export DUMONT_CONFIG=$HOME/.dumont/dumont.json; cd \"{wt}\" && "
        ...
        cmd = (f"{env}cat {specfile} | codex exec -m {cxmodel} "
...
def run_llm_verifier(dt, sid, wt, task, tag):
    ...
    cmd = (f"{env}{OPATH}; cd \"{wt}\" && {pre}cat {specfile} | claude -p "
...
def run_task(dt, gh, sid, integration_branch, task, merge_lock, lane_sems, lane_key, opus_validate_default=True):
    ...
    for i in range(1, max_iters + 1):
        ...
        try:
            run_worker(dt, sid, wt, model, spec, f"{tid}-{i}", worker_timeout, allowed)
        except TimeoutError as e:
...
def main():
    ...
    if cfg.get("dry_run") or "--dry-run" in sys.argv:
        ...
        opus_share = sum(1 for t in cloud if t.get("worker_model", MINIMAX) == OPUS)
        dumont_share = len(cloud) - opus_share
        # rough per-model cost (USD per 1000 tokens, approximates current pricing)
        opus_cost, dumont_cost = 0.015, 0.002
        est = opus_share * total_iters * opus_cost + dumont_share * total_iters * dumont_cost
        print(f"[DRY RUN] ib={ib} | tasks={len(cloud)} | iters={total_iters} | "
              f"opus={opus_share} dumont={dumont_share} | est_cost=${est:.2f}")
        return

    # INTEGRATION BRANCH VALIDATION: ensure IB exists on remote before any task runs
    try:
        if not gh.ref_exists(f"heads/{ib}"):
            raise SystemExit(f"integration_branch '{ib}' not found on remote")
    except SystemExit:
        raise
    except Exception as e:
        raise SystemExit(f"integration_branch '{ib}' not found on remote (gh api error: {e})")
    ...
    lane_sems = {}
    done_count = [0]          # mutable counter for progress closure
    errors_count = [0]
    t_start = [time.time()]   # mutable for closure
    ...
    def _print_progress():
        ...
        log(f"PROGRESS {done}/{total} done ({errs} errors, ETA {eta:.0f}s)")

    def _atexit_dump():
        log("ATEXIT: dumping state on crash/interrupt")
        with state_lock:
            tmp = state_path(ib) + f".atexit-{int(time.time()*1000)}"
            try:
        ...
        summary = {
            ...
            "escalations_dumont_claude": metrics.get("dumont_to_claude_escalations", 0),
            "est_cost_impact": metrics.get("dumont_to_claude_escalations", 0) * 0.013,
        }
        print(f"ATEEXIT summary: {json.dumps(summary)}", flush=True)

    atexit.register(_atexit_dump)

    pool = cfg.get("pool_sids") if cfg.get("pool_sids") is not None else [cfg["golden_sid"]]
    ...
    metrics = {"provisioned": 0, "task_retries": 0, "dumont_to_claude_escalations": 0}
    ...
    def provision_fresh():
        ...
        try:
            ...
            secrets = inject_into_sandbox(dt, nsid, ght)
            if secrets and not _SECRETS:
    ...
    try:
        ...
        for sid in pool:
            try:
                dt.start(sid)
                secrets = inject_into_sandbox(dt, sid, ght)
                if secrets and not _SECRETS:
                    for k, v in secrets.items():
                        if v:
                            _SECRETS[k] = v
        ...
        def worker(task):
                ...
                while True:
                    ...
                    try:
                        ...
                        if new_lane != old_lane:
                            with metrics_lock:
                                metrics["dumont_to_claude_escalations"] += 1
            ...
            if r.get("status") == "ERROR":
                with metrics_lock:
                    errors_count[0] += 1
            with state_lock:
                duration_s = round(time.time() - t_task_start, 1)
                r["duration_seconds"] = duration_s
                r["iterations"] = r.get("iters", 1)
                state[task["id"]] = r
    ...
    merged = sum(1 for r in results if r.get("status") == "MERGED")
    errors = sum(1 for r in results if r.get("status") == "ERROR")
    escalations = metrics.get("dumont_to_claude_escalations", 0)
    est_cost_impact = escalations * 0.013
    for r in results:
        print(json.dumps(r))
    print(f"\n{merged}/{len(results)} merged | {errors} errors | "
          f"dumont->claude escalations: {escalations} (est cost impact: ${est_cost_impact:.2f}) | "
          f"fresh sandboxes: {metrics['provisioned']} | retries: {metrics['task_retries']} | "
